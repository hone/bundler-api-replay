    ...
    context "when a router log" do
      ...
      it "enqueues jobs if a router request" do
        ...
        [replay_job, store_job].each do |job|
          ...
          expect(job.path).to eq("bundler-api.herokuapp.com/api/v1/dependencies?gems=SexpProcessor,oauth2,roauth,ruby_core_source,simple_oauth,windows-api,win32-api,bones-rcov,bones-rubyforge,bones-rspec,bones-zentest")
        end
      end
    end
